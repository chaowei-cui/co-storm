...
class Encoder:
    ...
    def __init__(
        ...
        elif encoder_type.lower() == "volcano":
            self.embedding_model_name = "ep-20250228235419-n5vjc"
            self.kargs = {
                "api_key": api_key or os.getenv("VOLCANO_API_KEY"),
                "api_base": api_base or os.getenv("VOLCANO_API_BASE"),
            }
        else:
            raise ValueError(
    ...
    def _get_single_text_embedding(self, text):
        ...
        # )
        client = OpenAI(
            api_key=self.kargs["api_key"],
            base_url=self.kargs["api_base"]+"/v3",
        )
