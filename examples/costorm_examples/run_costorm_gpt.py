...
from knowledge_storm.lm import OpenAIModel, VLLMClient, AzureOpenAIModel, DeepSeekModel
...
def main(args):
    ...
    ModelClass = (
        OpenAIModel if os.getenv("OPENAI_API_TYPE") == "openai" else DeepSeekModel
    )
    # If you are using Azure service, make sure the model name matches your own deployed model name.
    # The default name here is only used for demonstration and may not match your case.
    gpt_4o_mini_model_name = "gpt-4o-mini"
    gpt_4o_model_name = os.getenv("GPT_MODEL_NAME")
    if os.getenv("OPENAI_API_TYPE") == "azure":
        openai_kwargs["api_base"] = os.getenv("AZURE_API_BASE")
        openai_kwargs["api_version"] = os.getenv("AZURE_API_VERSION")
    elif os.getenv("OPENAI_API_TYPE") == "volcano":
        openai_kwargs["api_base"] = os.getenv("VOLCANO_API_BASE")
        openai_kwargs["api_key"] = os.getenv("VOLCANO_API_KEY")
    ...
    # warm start the system：原始预热对话、修改后的预热对话、预热专家列表
